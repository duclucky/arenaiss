  ...
  return (
    <div className="anim-fade" style={{ maxWidth: 640, margin: '0 auto', padding: '50px 22px', textAlign: 'center' }}>
      ...
      <p style={{ color: 'var(--text-sub)' }}>
        {won ? 'Your deck carried the fight — stat choice and type counters mattered.' : 'Close one. Try reordering your deck or leaning harder into type advantage.'}
      </p>

      <div className="panel" style={{ padding: 24, margin: '26px 0', display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.14em', color: 'var(--text-dim)' }}>REWARD</div>
        <div className="tabnums" style={{ fontSize: 40, fontWeight: 900, color: 'var(--accent)' }}>+{reward}</div>
        <div style={{ fontSize: 12, color: 'var(--text-sub)' }}>virtual credits · new balance: <b className="tabnums">{state.credits}</b></div>
      ...
      {battle && (
        <div className="panel" style={{ padding: '14px 18px', marginBottom: 22, textAlign: 'left' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-sub)', margin: 0 }}>
            {battle.rounds[battle.rounds.length - 1]?.reason ?? 'The battle is deterministic from deck order and stat choices.'}
          </p>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        ...
        <button className="btn" onClick={rematch} disabled={deck.length !== 5}>Rematch</button>
        <button className="btn btn-ghost" onClick={() => dispatch({ type: 'GOTO', screen: 'roster' })}>Collection</button>
      </div>
    </div>
  );
}
