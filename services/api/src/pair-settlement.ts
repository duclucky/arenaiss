...
export class PairSettlementWorker {
  ...
  async tick(): Promise<void> {
    ...
    for (const room of rows.filter((item) => item.state === 'OPEN' || item.state === 'JOINING' || item.state === 'JOINED')) {
      const owner = randomUUID();
      if (this.runtime.claimLease('pair-room-worker-leases', room.roomId, room.roomId, owner, Date.now(), 10 * 60_000) === 'BUSY') continue;
  ...
  private async progress(room: PairRoom): Promise<void> {
    ...
    if (chain.state === 3) {
      ...
      this.runtime.put('pair-rooms-v1', room.roomId, { ...room, state: 'SETTLED', ...(intent ? { verdictTx: intent.verdictTx } : {}) });
      return;
    }
    if (chain.state === 4) {
      this.runtime.put('pair-rooms-v1', room.roomId, { ...room, state: 'REFUNDABLE' });
      return;
    }
    if (room.state === 'JOINING') {
      if (chain.state === 2 && room.joinTx) {
        this.runtime.put('pair-rooms-v1', room.roomId, { ...room, state: 'JOINED' });
        return;
      }
    ...
    if (!intent) {
      ...
      if (outcome.state !== 'FINAL' || outcome.result === 'TIE') return;
    ...
    this.runtime.put('pair-rooms-v1', room.roomId, { ...room, state: 'SETTLED', verdictTx: intent.verdictTx, settleTx });
  }
  ...
  private defer(roomId: string, message: string): void {
    ...
    this.runtime.put('pair-room-worker-retries', roomId, { failures, nextAt: this.now() + 600 * failures });
