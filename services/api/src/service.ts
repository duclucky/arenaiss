...
export class ArenaApiService {
  ...
  private publicCampaign(campaign: SoloCampaignRecord): PublicEvaluationCampaign {
    const agentName = [...this.agents.values()].find((agent) => agent.versions.some((version) => version.agentsVersion === campaign.agent.versionId))?.name;
