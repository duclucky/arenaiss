  ...
  async handle(request: ApiRequest): Promise<ApiResponse> {
    try {
      ...
      if (request.method === 'POST' && request.path === '/api/marketplace/eligibility') {
        ...
        if (certificate.state === 'APPROVED') return this.json(201, certificate);
        const transaction = await this.marketplaceChain.approveEligibility({ digest: certificate.certificateDigest, agentId: certificate.agentId, version: certificate.agentVersionId, commitment: certificate.agentsCommitment, validUntil: certificate.expiresAt });
