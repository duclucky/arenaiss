  ...
  async handle(request: ApiRequest): Promise<ApiResponse> {
    try {
      ...
      if (request.method === 'POST' && request.path === '/api/marketplace/eligibility') {
        ...
        const certificate = reusable.find((row) => row.state === 'APPROVED') ?? reusable.find((row) => row.state === 'ELIGIBLE')
          ?? this.service.createMarketplaceEligibility(owner, eligibilityInput);
        const transaction = await this.marketplaceChain.approveEligibility({ digest: certificate.certificateDigest, tokenId: certificate.erc8004TokenId, agentId: certificate.agentId, version: certificate.agentVersionId, commitment: certificate.agentsCommitment, validUntil: certificate.expiresAt });
        return this.json(201, this.service.approveMarketplaceEligibility(this.service.operatorAddress, certificate.certificateDigest, transaction));
      }
      const certificateMatch = request.path.match(/^\/api\/marketplace\/certificates\/(sha256:[0-9a-fA-F]{64})\/approve$/);
      if (request.method === 'POST' && certificateMatch) { const operator = this.requireSession(request.headers); if (operator.toLowerCase() !== this.service.operatorAddress || !this.marketplaceChain?.approveEligibility) throw new Error('unauthorized operator'); const certificate = this.service.listMarketplaceCertificatesForOperator(operator).find((row) => row.certificateDigest === certificateMatch[1]); if (!certificate) throw new Error('marketplace certificate not found'); const transaction = await this.marketplaceChain.approveEligibility({ digest: certificate.certificateDigest, tokenId: certificate.erc8004TokenId, agentId: certificate.agentId, version: certificate.agentVersionId, commitment: certificate.agentsCommitment, validUntil: certificate.expiresAt }); return this.json(200, this.service.approveMarketplaceEligibility(operator, certificate.certificateDigest, transaction)); }
      ...
      if (request.method === 'POST' && request.path === '/api/marketplace/listings') {
        ...
        const certificate = this.service.listOwnedMarketplaceCertificates(session.principal).find((row) => row.certificateDigest === intent.certificateDigest);
        if (!certificate) throw new Error('marketplace certificate not found');
        const authorization = await this.marketplaceChain.approveEligibility({ digest: certificate.certificateDigest, tokenId: certificate.erc8004TokenId, agentId: certificate.agentId, version: certificate.agentVersionId, commitment: certificate.agentsCommitment, validUntil: certificate.expiresAt });
        this.service.approveMarketplaceEligibility(this.service.operatorAddress, certificate.certificateDigest, authorization);
        const transaction = intent.transaction ?? await this.managedIdentity.marketplaceCreateListing(session.userId!, { tokenId: intent.tokenId, agentId: intent.agentId, version: intent.agentsVersion, commitment: intent.agentsCommitment, certificateDigest: intent.certificateDigest, price: intent.price, expiresAt: intent.expiresAt, nftApprovalIdempotencyKey: intent.nftApprovalIdempotencyKey, listingIdempotencyKey: intent.listingIdempotencyKey });
      ...
      if (request.method === 'POST' && listingCancel) {
        ...
        const listing = this.service.listOwnedMarketplaceListings(session.principal).find((row) => row.listingId === listingCancel[1]);
