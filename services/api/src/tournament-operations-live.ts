...
export function tournamentOperationsFromEnvironment(environment: NodeJS.ProcessEnv, runtime: SqliteRuntimeStore, service: ArenaApiService, operatorAddress: string): TournamentOperationsPort | undefined {
  ...
  const provider = new OpenAICompatibleEvaluationProvider({ ...values.routing.provider, timeoutMs: 300_000 });
  ...
  const orchestrator = new TournamentOrchestrator(new TournamentEvaluationPairRunner(provider, { model: values.routing.model, maxOutputTokens: 1_500, temperature: 0.2 }, runtime), new TournamentComparisonJudgeAdapter(tracker));
