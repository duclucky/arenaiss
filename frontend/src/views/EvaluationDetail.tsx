...
export function EvaluationDetail() {
  ...
  return <section className="mx-auto max-w-5xl space-y-8">
    ...
    {campaign.state === 'RECOVERY_REQUIRED' && <p role="status" className="glass-panel p-4 text-sm text-amber-900"><strong>GenLayer submission needs reconciliation.</strong> Its transaction result is unknown, so this campaign has no final score. The Evo fee remains in escrow until the transaction is resolved or the onchain timeout refund becomes available.</p>}
    {fee?.state === 'REFUNDED' && <div role="status" className="glass-panel flex flex-wrap items-center justify-between gap-3 p-4"><p><strong>{fee.amountUsdc} USDC refunded</strong> to the payer from Evo escrow.</p>{fee.settlement?.txHash && <a className="font-semibold underline" href={`${networkExplorer(fee.escrowAddress, fee.settlement.txHash)}`} target="_blank" rel="noreferrer">View Arc refund receipt <ExternalLink className="inline" size={14} /></a>}</div>}
    {feeLoaded && fee === null && <p className="glass-panel p-4 text-sm text-neutral-700">Legacy campaign, no Evo escrow record.</p>}
    {fee?.state === 'HELD' && <div role="status" className="glass-panel flex flex-wrap items-center justify-between gap-4 p-4 text-sm"><div><p><strong>{fee.amountUsdc} USDC remains held</strong> in Evo escrow.</p><p className="mt-1 text-neutral-700">{fee.refundAvailableAt ? `Timeout refund available ${new Date(fee.refundAvailableAt * 1000).toLocaleString()}.` : 'Waiting for the confirmed deposit time.'}</p></div>{fee.refundAvailableAt && <button type="button" className="metal-button-solid" disabled={refunding || Date.now() < fee.refundAvailableAt * 1000 || !evaluationApi?.claimTimeoutRefund} onClick={async () => { if (!id || !evaluationApi?.claimTimeoutRefund) return; setRefunding(true); setError(''); try { setFee(await evaluationApi.claimTimeoutRefund(id, crypto.randomUUID())); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Timeout refund failed. Check the Arc receipt before retrying.'); } finally { setRefunding(false); } }}>{refunding ? 'Claiming refund…' : `Claim ${fee.amountUsdc} USDC timeout refund`}</button>}</div>}
