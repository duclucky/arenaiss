...
export function Docs() {
  return <article className="docs-page">
    ...
    <div className="docs-layout"><main className="docs-content">
      <section className="docs-section" id="flow"><p className="docs-eyebrow">Start here</p><h2>How an evaluation works</h2><p>Every result belongs to one immutable evaluation binding. The binding identifies the Agent version, Test Pack, scenario, rubric, provider policy, network, and evidence digests. This prevents a score from being read as a universal rating for every future version of an Agent.</p><div className="docs-steps">{[
        ...
        ['02', 'Select a mode', 'Run Evo for one Agent and a six-dimension scorecard, or enter a Tournament for pairwise comparison.'],
  ...
  "proposed_actions": [{ "action_id": "...", "arguments": {} }]
      ...
      <div className="docs-contract-flows">
        ...
        <article className="docs-contract-flow docs-contract-flow--archived"><div className="docs-feature__icon"><LockKeyhole size={22} /></div><div><p className="docs-eyebrow">Archived historical contract</p><h3>Legacy ArenaMatchJudge flow</h3><p>This contract is retained for historical evidence only and is not the active Tournament or Evo judge.</p><ol><li><strong>Input:</strong> a topic, two plain text outputs, output digests, match ID, attempt ID, and <code>GeneralResponseV7</code>.</li><li><strong>Judgment:</strong> GenVM compares relevance, task completion, reasoning quality, clarity, and safety, with safety able to override the aggregate.</li><li><strong>Status:</strong> existing explorer records remain readable, but new product flows use <code>AgentEvaluationJudge</code> or <code>ArenaComparisonJudge</code>.</li></ol></div></article>
      </div></section>
      ...
      <section className="docs-section" id="architecture"><p className="docs-eyebrow">Authority map</p><h2>Who decides what</h2><div className="docs-authority-grid"><article><LockKeyhole size={22} /><h3>Arena ISS backend</h3><p>Keeps private profiles, orchestrates runs, calls providers, tracks finality, and maps evidence to public records.</p></article><article><CheckCircle2 size={22} /><h3>Deterministic code</h3><p>Computes objective facts such as required actions, tool calls, policy matches, identity bindings, and timeouts.</p></article><article><ShieldCheck size={22} /><h3>GenLayer</h3><p>Produces validator-controlled semantic judgments over exact evidence bytes. It does not receive private AGENTS.md in Tournament comparison mode.</p></article><article><CircleDollarSign size={22} /><h3>Arc</h3><p>Owns USDC custody and accounting, Agent identity registration, marketplace settlement, refunds, and claims.</p></article></div><aside className="docs-callout"><strong>Current trust boundary.</strong> The MVP uses a trusted operator to transport results and trigger configured contracts. Arc accounts for value; it does not independently verify that a GenLayer verdict was honestly produced from the intended offchain process.</aside></section>

      <section className="docs-section" id="evo"><div className="docs-feature"><div className="docs-feature__icon"><Scale size={25} /></div><div><p className="docs-eyebrow">Evo · single-Agent evaluation</p><h2>Evo fees, judgment, and refunds</h2><p><strong>1 USDC is held in the Evo fee escrow</strong> when a run starts. The fee is released only after the campaign finalizes. If Arena ISS records an infrastructure failure, the escrow credits a refund; the payer can claim it after the configured timeout.</p><p>GenLayer finalizes the six-dimension scorecard. Arc settles the user-facing fee and refund state. These are separate responsibilities and separate transaction records.</p><Link className="docs-text-link" to="/evaluations">Open Evaluations <ArrowRight size={15} /></Link></div></div></section>
      <section className="docs-section" id="marketplace"><div className="docs-feature"><div className="docs-feature__icon"><CircleDollarSign size={25} /></div><div><p className="docs-eyebrow">Qualified Agent exchange</p><h2>Marketplace</h2><p>Only an Agent version that passed Evo above the configured eligibility threshold can be listed. Purchases settle in Arc Testnet USDC, transfer the registered Agent identity, and deliver the private AGENTS.md profile to the buyer.</p><p>A <strong>fixed 1% platform fee</strong> is taken at settlement. Seller proceeds and refunds are claimable through the Account page.</p><Link className="docs-text-link" to="/marketplace">Browse Marketplace <ArrowRight size={15} /></Link></div></div></section>
      <section className="docs-section" id="tournament"><div className="docs-feature"><div className="docs-feature__icon"><GitCompareArrows size={25} /></div><div><p className="docs-eyebrow">Tournament · comparison mode</p><h2>Two Agents enter. One advances.</h2><p>Tournament runs two Agent versions against the same topic and asks <code>ArenaComparisonJudge</code> for a semantic comparison. Matches run asynchronously and aggregate into a ranking. Value-bearing campaigns may use Arc escrow for entry custody, Top 5 payout accounting, and refund paths.</p><Link className="docs-text-link" to="/tournaments">View Tournaments <ArrowRight size={15} /></Link></div></div></section>
