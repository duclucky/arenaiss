...
function CampaignList({ campaigns }: { campaigns: EvaluationCampaign[] }) {
  const ordered = campaigns.slice().sort((a, b) => (b.createdAt ?? 0) - (a.createdAt ?? 0));
  ...
  return <section aria-labelledby="campaigns-heading"><h2 id="campaigns-heading" className="mb-4 text-2xl font-bold">My evaluations</h2>{campaigns.length === 0 ? <div className="glass-panel p-7 text-neutral-700">No evaluation records yet.</div> : <ul className="space-y-3">{ordered.map((campaign) => <li key={campaign.campaignId} className="glass-panel flex flex-wrap items-center justify-between gap-4 p-5"><div className="min-w-0"><p className="page-kicker">Evo evaluation</p><p className="mt-2 font-semibold">{campaign.createdAt && Number.isFinite(campaign.createdAt) ? <time dateTime={new Date(campaign.createdAt).toISOString()}>{utcDate.format(campaign.createdAt)} UTC</time> : 'Date unavailable'}</p></div><Link aria-label="Open evaluation results" className="metal-button-ghost" to={`/evaluations/${campaign.campaignId}`}>{campaign.state} →</Link></li>)}</ul>}</section>;
}
