...
function CampaignList({ campaigns }: { campaigns: EvaluationCampaign[] }) { return <section aria-labelledby="campaigns-heading"><h2 id="campaigns-heading" className="mb-4 text-2xl font-bold">My evaluations</h2>{campaigns.length === 0 ? <div className="glass-panel p-7 text-neutral-700">No evaluation records yet.</div> : <ul className="space-y-3">{campaigns.map((campaign) => <li key={campaign.campaignId} className="glass-panel flex flex-wrap items-center justify-between gap-4 p-5"><div className="min-w-0"><p className="page-kicker">Evo evaluation</p><p className="mt-2 font-semibold">Pack {campaign.packVersion} · {campaign.items.length} test{campaign.items.length === 1 ? '' : 's'}</p></div><Link aria-label="Open evaluation results" className="metal-button-ghost" to={`/evaluations/${campaign.campaignId}`}>{campaign.state} →</Link></li>)}</ul>}</section>; }
