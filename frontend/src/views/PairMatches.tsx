  ...
  return <section className="mx-auto max-w-5xl space-y-8">
    ...
    <section aria-labelledby="pair-rooms-heading"><h2 id="pair-rooms-heading" className="mb-4 text-2xl font-semibold">{viewCopy.title}</h2>
      {roomsError ? <div className="glass-panel p-6" role="alert"><p>{displayLabel(roomsError)}</p><button type="button" className="metal-button-ghost mt-4" onClick={() => void refresh().catch(() => undefined)}>Retry rooms</button></div>
        ...
        : visibleRooms.length === 0 ? <p className="glass-panel p-6">{viewCopy.empty}</p> : <ul aria-label={`${viewCopy.title} list`} className="grid items-start gap-4 md:grid-cols-2 xl:grid-cols-3">{visibleRooms.map((room, roomIndex) => { const outcome = settledOutcome(room); return <li className={`glass-panel h-full space-y-3 p-4 ${expandedVerdict === room.roomId ? 'md:col-span-2 xl:col-span-3' : ''}`} key={room.roomId}>
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="page-kicker">Room #{roomIndex + 1}</p><strong className="mt-1 block">{units(room.stake)} USDC each</strong></div><div className="flex flex-wrap items-center gap-2">{outcome && <span className={`pair-outcome ${outcome === 'YOU WON' ? 'pair-outcome--won' : 'pair-outcome--lost'}`}>{outcome}</span>}<span className="retro-chip px-3 py-1 text-xs">{displayLabel(room.state)}</span></div></div>
