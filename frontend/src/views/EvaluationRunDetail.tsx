...
export function EvaluationRunDetail() {
  ...
  if (error) return <div role="alert" className="glass-panel mx-auto max-w-4xl p-8">{displayLabel(error)}</div>;
  ...
  return <section className="mx-auto max-w-4xl space-y-8">
    ...
    <div className="glass-panel p-6"><div className="grid gap-3 sm:grid-cols-2"><Fact label="Topic" value={topicName(run.scenario.scenarioId)}/><Fact label="Mode" value={displayLabel(run.mode)}/><Fact label="Agent runtime state" value={displayLabel(run.provider.state)}/>{run.provider.route && <Fact label="Runtime route" value={run.provider.route === 'FALLBACK' ? 'Fallback' : 'Primary'}/>}{run.provider.model && <Fact label="Runtime model" value={run.provider.model}/>}<Fact label="Judge" value="GenLayer GenVM"/><Fact label="GenLayer finality" value={displayLabel(run.judge.state)}/><Fact label="Rubric" value={run.rubricVersion}/></div><p className="mt-4 text-xs text-neutral-600">The runtime model generates the Agent response. GenLayer GenVM validators independently judge the submitted evidence. Topic labels describe the test area; the exact prompt and hidden context remain private.</p>{receiptUrl && <a className="metal-button-ghost mt-5 w-full gap-2 sm:w-auto" href={receiptUrl} target="_blank" rel="noreferrer">View Studio Next transaction <ExternalLink size={15} aria-hidden="true" /></a>}</div>
    {run.scorecard ? <section className="glass-panel p-6" aria-labelledby="scorecard-heading"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="page-kicker">Verdict</p><h2 id="scorecard-heading" className="text-2xl font-bold">{displayLabel(run.scorecard.resultClass)}</h2></div><p className="font-mono text-4xl">{run.scorecard.overallScore}<span className="text-lg text-neutral-600">/100</span></p></div>{run.scorecard.resultClass === 'FAIL' && run.scorecard.overallScore === 0 && <p role="status" className="mt-4 border-l-2 border-black pl-4 text-sm leading-relaxed"><strong>A policy finding or failing safety/rule grade sets the effective score to 0.</strong> The dimension grades below remain available for audit.</p>}<div className="mt-6 overflow-x-auto"><table aria-label="Score dimensions" className="w-full min-w-[420px] border-collapse text-left text-sm"><thead className="border-y border-black bg-black/5 text-xs uppercase tracking-wider"><tr><th className="p-3">Dimension</th><th className="p-3 text-right">Grade</th></tr></thead><tbody>{run.scorecard.dimensions.map((dimension) => <tr className="border-b border-black/15" key={dimension.dimensionId}><td className="p-3">{displayLabel(dimension.dimensionId)}</td><td className="p-3 text-right font-bold">{displayLabel(dimension.grade)}</td></tr>)}</tbody></table></div></section> : <div className="glass-panel p-6">Scorecard is not final yet.</div>}
    ...
    <div className="border-l-2 border-black pl-4 text-xs leading-relaxed text-neutral-600"><strong className="text-black">Privacy boundary.</strong> Only the Agent owner can view the output and detailed findings. Public records exclude prompts, hidden context, runtime output and private reasons.</div>
  </section>;
}
