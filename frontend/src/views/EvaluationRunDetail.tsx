...
export function EvaluationRunDetail() {
  ...
  return <section className="mx-auto max-w-4xl space-y-8">
    ...
    <div className="glass-panel p-6"><div className="grid gap-3 sm:grid-cols-2"><Fact label="Topic" value={topicName(run.scenario.scenarioId)}/><Fact label="Mode" value={run.mode}/><Fact label="Agent runtime state" value={run.provider.state}/>{run.provider.route && <Fact label="Runtime route" value={run.provider.route === 'FALLBACK' ? 'Fallback' : 'Primary'}/>}{run.provider.model && <Fact label="Runtime model" value={run.provider.model}/>}<Fact label="Judge" value="GenLayer GenVM"/><Fact label="GenLayer finality" value={run.judge.state}/><Fact label="Rubric" value={run.rubricVersion}/></div><p className="mt-4 text-xs text-neutral-600">The runtime model generates the Agent response. GenLayer GenVM validators independently judge the submitted evidence. Topic labels describe the test area; the exact prompt and hidden context remain private.</p>{receiptUrl && <a className="metal-button-ghost mt-5 w-full gap-2 sm:w-auto" href={receiptUrl} target="_blank" rel="noreferrer">View Studio Next transaction <ExternalLink size={15} aria-hidden="true" /></a>}</div>
