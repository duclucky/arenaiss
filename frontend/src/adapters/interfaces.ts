...
export type EvaluationCampaign = { schema: string; campaignId: string; agentVersionId: string; agentName?: string; packId: string; packVersion: string; rubricVersion: string; state: string; createdAt?: number; startedAt?: number; items: Array<{ scenarioId: string; state: string; attempt: number; runIds: string[]; score?: string; overallScore?: number; providerModel?: string; providerRoute?: 'PRIMARY' | 'FALLBACK'; failureStage?: string; failureCode?: string }> };
export type EvaluationRun = { schema: string; runId: string; agentVersionId: string; mode: string; rubricVersion: string; scenario: { scenarioId: string; version: string; mode: string; digest: string }; provider: { state: string; model?: string; route?: 'PRIMARY' | 'FALLBACK' }; judge: { state: string; transactionHash?: string }; scorecard?: { resultClass: string; overallScore: number; dimensions: Array<{ dimensionId: string; grade: string }>; actionsExecuted: false } };
export type PrivateEvaluationRun = { schema: string; provider: { state: string; model?: string; route?: 'PRIMARY' | 'FALLBACK'; output?: unknown }; scorecard?: Record<string, unknown> };
...
export interface EvaluationApiAdapter {
  ...
  getRun(runId: string, privateView?: boolean): Promise<EvaluationRun | PrivateEvaluationRun>;
