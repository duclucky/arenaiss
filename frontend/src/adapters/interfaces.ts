...
export type EvaluationCampaign = { schema: string; campaignId: string; agentVersionId: string; packId: string; packVersion: string; rubricVersion: string; state: string; createdAt?: number; startedAt?: number; items: Array<{ scenarioId: string; state: string; attempt: number; runIds: string[]; score?: string; overallScore?: number; providerModel?: string; providerRoute?: 'PRIMARY' | 'FALLBACK'; failureStage?: string; failureCode?: string }> };
