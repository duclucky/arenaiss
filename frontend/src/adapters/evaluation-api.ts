...
export class HttpEvaluationAdapter implements EvaluationApiAdapter {
  ...
  async getCampaign(id: string) { const r = await this.fetcher(`${this.base}/api/evaluation-campaigns/${encodeURIComponent(id)}`, { credentials: 'omit' }); if (r.status === 404) return null; return this.read<EvaluationCampaign>(r); }
  ...
  private async request<T>(path: string, init: RequestInit = {}) { return this.read<T>(await this.fetcher(`${this.base}${path}`, { ...init, credentials: init.credentials || 'include' })); }
