...
export class HttpEvaluationAdapter implements EvaluationApiAdapter {
  ...
  async getFee(campaignId: string) { await this.auth(); return this.request<Awaited<ReturnType<NonNullable<EvaluationApiAdapter['getFee']>>>>(`/api/evaluation-campaigns/${encodeURIComponent(campaignId)}/fee`); }
