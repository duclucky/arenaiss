...
export class HttpEvaluationAdapter implements EvaluationApiAdapter {
  ...
  async getExecutionConfig() { return this.request<{ enabled: boolean; feeUsdc?: string; feeAsset: 'USDC'; feeCustody?: 'ESCROW'; escrowAddress?: string; genLayerGasPayer: 'OWNER' }>('/api/evaluation-config'); }
  ...
  async getFee(campaignId: string) { await this.auth(); const response = await this.fetcher.call(globalThis, `${this.base}/api/evaluation-campaigns/${encodeURIComponent(campaignId)}/fee`, { credentials: 'include' }); return response.status === 204 ? null : this.read<NonNullable<Awaited<ReturnType<NonNullable<EvaluationApiAdapter['getFee']>>>>>(response); }
  async claimTimeoutRefund(campaignId: string, idempotencyKey: string) { await this.auth(); return this.request<Awaited<ReturnType<NonNullable<EvaluationApiAdapter['claimTimeoutRefund']>>>>(`/api/evaluation-campaigns/${encodeURIComponent(campaignId)}/fee/timeout-refund`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ idempotencyKey }) }); }
