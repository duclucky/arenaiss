...
describe('Arena ISS visual shell', () => {
  ...
  it('renders the Arena ISS platform message without duplicate page actions', async () => {
    ...
    expect(ticker.querySelectorAll('.header-ticker-group')).toHaveLength(3);
    expect(ticker.querySelectorAll('.header-ticker-group[aria-hidden="true"]')).toHaveLength(2);
    expect(ticker.querySelector('[data-brand-logo="arc"]')).toHaveAttribute('viewBox', '0 0 47.72 50');
    expect(ticker.querySelector('[data-brand-logo="usdc"]')).toHaveAttribute('viewBox', '0 0 96 96');
    expect(ticker.querySelector('[data-brand-logo="genlayer"]')).toHaveAttribute('viewBox', '0 0 97.76 91.93');
    expect(ticker.querySelector('.header-ticker-track')).toHaveAttribute('data-motion', 'continuous');
