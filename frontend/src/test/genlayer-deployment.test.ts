...
describe('Studio Next deployment verification', () => {
  it('invokes the browser fetch function with its global receiver', async () => {
    ...
    await expect(verifyStudioNextDeployments(config, fetcher as typeof fetch, { evaluationJudge: hash, comparisonJudge: hash })).resolves.toEqual({
      state: 'VERIFIED', chainId: 61997, evaluationJudgeVerified: true, comparisonJudgeVerified: true,
    });
  });
  ...
  it('stops before contract reads on another chain', async () => {
    ...
    await expect(verifyStudioNextDeployments(config, fetcher as typeof fetch)).resolves.toEqual({
      state: 'WRONG_CHAIN', chainId: 61999, evaluationJudgeVerified: false, comparisonJudgeVerified: false,
    });
