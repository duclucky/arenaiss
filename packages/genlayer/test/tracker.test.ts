...
test("raw and normalized receipt shapes map to one lifecycle", () => {
  ...
  assert.deepEqual(normalizeReceipt({ statusName: "FINALIZED", resultName: "MAJORITY_DISAGREE", txExecutionResultName: "FINISHED_WITH_RETURN" }), { finality: "FINALIZED", execution: "FAILED", failureReason: "NO_CONSENSUS" });
  assert.deepEqual(normalizeReceipt({ status: "FINALIZED", result_name: "MAJORITY_DISAGREE", execution_status: "SUCCESS" }), { finality: "FINALIZED", execution: "FAILED", failureReason: "NO_CONSENSUS" });
  assert.deepEqual(normalizeReceipt({ statusName: "UNDETERMINED", txExecutionResultName: "FINISHED_WITH_RETURN" }), { finality: "FINALIZED", execution: "FAILED", failureReason: "NO_CONSENSUS" });
