...
The pair-specific worker reuses the Evaluation provider envelope and rich GenLayer ComparisonRun tracker. It binds both exact Agent versions and a fixed common scenario to the room, accepts only a canonical finalized `A_WIN` or `B_WIN`, persists one settlement intent, and reads Arc settlement back. Independent rooms progress concurrently so one slow provider request does not prevent a later joined room from starting. Public room records expose a bounded stage (`QUEUED`, Agent execution, GenLayer verdict, retry, Arc settlement, or complete) without exposing provider output or private evidence. Participant-visible failure codes are limited to `PROVIDER_ERROR`, `GENLAYER_BUSY`, `GENLAYER_ERROR`, `VERDICT_PENDING`, and `ARC_ERROR`; raw upstream messages remain in the private runtime error store. Ties and exhausted provider retries leave funds available for deadline refunds; the worker also opens overdue refunds.
