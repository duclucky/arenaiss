...
- Cancellation: the creator may cancel an `OPEN` room and recover the full stake. Once joined, both players normally request early cancellation. For a provider failure, GenLayer capacity failure, invalid/failed comparison, no-consensus result, or tie, the worker uses the two managed participant wallets to submit both cancellation requests immediately. It then attempts both withdrawals independently; a failed delivery leaves that participant's exact stake as pull credit. Deadline expiry remains the recovery path when the automatic contract calls cannot be completed.
