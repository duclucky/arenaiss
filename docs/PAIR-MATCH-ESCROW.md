...
This is an `EVAL-7` value-bearing slice for one independent two-Agent room. `PairMatchEscrow` was deployed on Arc Testnet at [`0xD7CB...c6c1`](https://testnet.arcscan.app/address/0xD7CB8dE4cED8F988152CDc51EBCf7a17c602c6c1), with [receipt and readback evidence](evidence/arc-testnet/pair-match-escrow-deployment-2026-09-17.json). It has **not** been audited. A two-wallet Arc Testnet run verified matching deposits, mutual cancellation, refund credits, and both withdrawals; the verdict-to-settlement path remains unverified. `ARENA_PAIR_ROOMS_ENABLED=1` and `ARC_PAIR_ESCROW_ADDRESS` are both required before the API exposes deposits; the API also reads Arc chain ID, deployed code, USDC and operator immutables before each write. A pair settlement worker is available only when the provider, finalized GenLayer comparison judge, operator signer, and managed identity configuration are present.
...
- Cancellation: the creator may cancel an `OPEN` room and recover the full stake. Once joined, both players normally request early cancellation. A finalized no-consensus result starts a new whole-pair attempt with the next distinct scenario, up to three retries after the initial attempt. If all four attempts lack consensus, or if the provider, GenLayer execution, or comparison pipeline fails, the worker uses the two managed participant wallets to submit both cancellation requests immediately. A semantic tie is already a consensus result and also opens refunds rather than being retried. The worker then attempts both withdrawals independently; a failed delivery leaves that participant's exact stake as pull credit. Deadline expiry remains the recovery path when the automatic contract calls cannot be completed.
...
The pair-specific worker reuses the Evaluation provider envelope and rich GenLayer ComparisonRun tracker. Every attempt binds both exact Agent versions to one common scenario; a no-consensus retry creates a new attempt ID and selects the next scenario without modifying either Agent artifact or reusing output from an earlier attempt. The worker accepts only a canonical finalized `A_WIN` or `B_WIN`, persists the winning attempt with one settlement intent, and reads Arc settlement back. Independent worker ticks may overlap, while per-room leases prevent duplicate work, so one slow provider request does not prevent a later joined room from starting. Public room records expose a bounded stage (`QUEUED`, Agent execution, GenLayer verdict, retry, refund, Arc settlement, or complete) without exposing provider output or private evidence. Participant-visible failure codes are limited to `PROVIDER_ERROR`, `GENLAYER_BUSY`, `GENLAYER_NO_CONSENSUS`, `GENLAYER_ERROR`, `VERDICT_PENDING`, and `ARC_ERROR`; raw upstream messages remain in the private runtime error store. Retry exhaustion, other evaluation failures, and ties create a durable automatic-refund intent, submit both managed-wallet cancellation requests, and attempt each withdrawal independently. A failed withdrawal remains claimable; unresolved Arc submission failures are retried per room and never block another room.

The website routes open rooms, the signed-in user's rooms, and that user's terminal rooms through `/pairs/open`, `/pairs/mine`, and `/pairs/completed`. Anonymous callers can list only currently joinable `OPEN` rooms; participant history is filtered by the authenticated principal in the API. The page and API never accept a client-supplied winner. Local tests cover access scoping, worker binding, settlement, retry budget, tie and timeout behavior. A live two-wallet deposit → verdict → settlement → withdrawal journey remains unverified. Deployment and this website release were authorized on 2026-09-17; future funding or participant transactions need their own authorization.
